    ...
    try:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                logger.info(f"Forwarding to inference service: {INFERENCE_SERVICE_URL}/predict")
                logger.info(f"Request details: filename={file.filename}, size={len(content)}")
                ...
                response = await client.post(f"{INFERENCE_SERVICE_URL}/predict", files=files)
